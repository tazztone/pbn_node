...
class SVGGenerator:
    ...
    def generate_svg(self, regions: Dict[int, Polygon], 
                    labels: LabelData, colors: ColorPalette) -> str:
        """
    ...
    def group_paths_by_color(self, regions: Dict[int, Polygon], 
                           colors: ColorPalette) -> Dict[str, List[int]]:
        """
    ...
    def _polygon_to_path(self, polygon: Polygon) -> str:
        ...
        path_parts = [f"M {coords[0][0]:.{self.coordinate_precision}f},{coords[0][1]:.{self.coordinate_precision}f}"]
